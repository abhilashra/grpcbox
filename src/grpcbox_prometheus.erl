...
observe_rpc_latency(server, Method, LatencyMicros) ->
    catch prometheus_counter:inc(?SERVER_LATENCY_MICS, [Method], round(LatencyMicros)),
    ok;
observe_rpc_latency(client, Method, LatencyMicros) ->
    catch prometheus_counter:inc(?CLIENT_LATENCY_MICS, [Method], round(LatencyMicros)),
    ok.
